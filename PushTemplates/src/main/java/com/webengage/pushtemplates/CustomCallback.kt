...
import com.webengage.pushtemplates.templates.ProductCatalogRenderer
...
class CustomCallback : CustomPushRerender, CustomPushRender {
    ...
    override fun onRender(context: Context?, pushNotificationData: PushNotificationData?): Boolean {
        ...
        else if(pushNotificationData.customData.containsKey(Constants.TEMPLATE_TYPE) && pushNotificationData.customData.getString(
                    Constants.TEMPLATE_TYPE
                ).equals(Constants.CATALOG))
            return ProductCatalogRenderer().onRender(context,pushNotificationData)
